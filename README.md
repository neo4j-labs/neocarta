...
![The semantic-memory subgraph grafted onto the catalog: a Task (secondary label Memory) owns embedded Phrase wordings via HAS_PHRASE and canonical Query nodes via HAS_QUERY, and each Query links to the Table and Column nodes it uses via USES_TABLE and USES_COLUMN](assets/images/architecture/semantic-memory.png)

Memory nodes
* `Task` (secondary label `Memory`) — a remembered analytical task, keyed by a CamelCase name, carrying free-form `observations`
