...
# ]
# ///
"""Create and verify the OpenAI External-Models embedding endpoint (mcp.md Steps B and C).
...
Databricks secret. Provision that secret first with ``setup_secrets.sh`` (mcp.md
Step A). Step C confirms the endpoint is READY and returns 1536-dim vectors
through the same ``ai_query`` path the dbxcarta Spark ingest uses.

The endpoint name, model, secret scope, and key default to the values mcp.md and
the finance-genie overlay use, so a plain run wires finance-genie's Option 1
cutover end to end.
...
# Defaults match mcp.md "Provisioning the OpenAI Endpoint" and the finance-genie
# overlay, so a bare run wires Option 1 without extra flags.
...
# The same probe string mcp.md Step C documents; any text works, the vector
# length is what the check reads.
...
def verify_endpoint(
    ...
    if dim != args.dimension:
        fail(
            ...
            f"{args.dimension}. Set DBXCARTA_EMBEDDING_DIMENSION to {dim} or check the model."
        )
...
def main() -> None:
    ...
    if not secret_present(client, args.secret_scope, args.secret_key):
        fail(
            ...
            "Provision it first (mcp.md Step A): "
