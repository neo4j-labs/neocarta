...
import neocarta.connectors.databricks.ingest.transform.embeddings as emb
from neocarta.connectors.databricks.ingest.transform.ledger import (
    read_ledger,
    split_by_ledger,
    upsert_ledger,
)
...
def embedded_batch(
    ...
    hash/embed mismatch. When `ledger_enabled`, rows whose embedding_text_hash
    and model already exist in the per-label ledger are served from the ledger
    (no ai_query call); only misses call the endpoint and the ledger is then
    upserted with the new vectors (excluding errors). The ledger is keyed by
    text-hash + model, so a per-batch call already scopes the read and the
    upsert — there is no global ledger read.
    ...
    `summary`; if this batch's failure count exceeds `embedding_failure_max`
    the run fails *before* the batch is yielded (so nothing is written). The
    transient path is always deleted on exit, success or failure, so nothing
...
def finalize_embedding_summary(summary: RunSummary) -> None:
    ...
    fields the summary wire shape carries. It is reporting only; nothing gates
    on it (the gate is the per-batch failure count).
    """
...
def _embed_with_ledger(
    ...
    if not settings.ledger_enabled:
        return emb.add_embedding_column(df, endpoint, dimension)
    ...
    embedded_misses = emb.add_embedding_column(misses_df, endpoint, dimension)
...
def _accumulate_and_gate(
    ...
    logger.info(
        "[databricks] %s batch embeddings: attempts=%d successes=%d failures=%d",
        label.value,
        attempts,
        successes,
        failures,
    ...
    cap = settings.embedding_failure_max
    if cap > 0 and failures > cap:
        raise RuntimeError(
            f"[databricks] {label.value} batch embedding failure count"
            f" {failures} exceeds NEOCARTA_DATABRICKS_EMBEDDING_FAILURE_MAX {cap};"
