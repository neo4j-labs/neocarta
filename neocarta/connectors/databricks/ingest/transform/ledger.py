...
Each embeddable label (Table, Column, Schema, Database) gets its own Delta
path so its embeddings can be invalidated independently.

Gated by `NEOCARTA_DATABRICKS_LEDGER_ENABLED` (off by default): `embed_stage`
imports these functions unconditionally but only calls them when the flag is on.
"""
...
def upsert_ledger(staged_df: DataFrame, ledger_root: str, label: NodeLabel) -> None:
    ...
    try:
        ...
        logger.info("[databricks] created ledger for %s at %s", label.value, ledger_path)
        return
    ...
    logger.info("[databricks] upserted ledger for %s at %s", label.value, ledger_path)
