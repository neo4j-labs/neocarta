...
def sample(
    ...
    if raw_df is None:
        logger.warning("[databricks] no value rows produced")
        return (
    ...
    for cand in candidates:
        ...
        for chunk_index, chunk_cols in enumerate(_chunk(cand.column_names, chunk_size)):
            try:
                ...
                table_failed = True
                logger.warning(
                    "[databricks] cardinality probe failed for %s chunk=%d cols=%s: %s",
                    cand.fq(),
                    chunk_index,
                    chunk_cols,
                    exc,
    ...
    for cand in candidates:
        for chunk_index, chunk_cols in enumerate(_chunk(cand.column_names, chunk_size)):
            try:
                ...
                logger.warning(
                    "[databricks] sampling failed for %s chunk=%d cols=%s: %s",
                    cand.fq(),
                    chunk_index,
                    chunk_cols,
                    exc,
...
def _filter_readable_schemas(
    ...
    for (cat, sch), cands in by_schema.items():
        ...
        if not readable_schema:
            skipped += 1
            logger.warning(
                "[databricks] skipping schema %s.%s (%d probes failed, last: %s)",
                cat,
                sch,
                len(probes),
                last_exc,
