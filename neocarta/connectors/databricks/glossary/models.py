"""Extract-stage typed dictionaries for the Databricks glossary connector.
...
``value_name=None`` so the tag still surfaces as a :Category.
"""
...
class TagPolicyValueInfo(TypedDict):
    ...
    values (it still becomes a :Category, with no :BusinessTerm children).
    """
