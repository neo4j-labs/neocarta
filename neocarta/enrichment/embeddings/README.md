...
| `dimensions` | `None` | Requested vector dimension for models that support truncation. When set it is sent to LiteLLM with `drop_params=True`, so models that don't support it drop the value and keep their native dimension. `None` = auto-detect. |
| `litellm_kwargs` | `None` | Extra keyword arguments forwarded to `litellm.embedding` / `litellm.aembedding`. Use this for `api_key` / `api_base` (LiteLLM Proxy or custom endpoints), `api_version`, or `additional_drop_params=["dimensions"]` for OpenAI-compatible endpoints LiteLLM can't introspect. |
