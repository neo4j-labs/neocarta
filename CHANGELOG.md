...
### Added
* Add the Collibra Data Catalog source connector (Core REST API v2), split into two data-type sub-connectors matching the connector contract: `CollibraSchemaConnector` (Communities → Database, physical Domains → Schema, Table/Column assets) and `CollibraGlossaryConnector` (business-glossary Domains → Glossary, Data Category → Category, Business Term → BusinessTerm, plus `TAGGED_WITH` tags). Both expose `extract`/`transform`/`load`/`ingest` (+ deprecated `run`), `include_nodes`/`include_relationships` filtering, and community/domain/asset-type scoping; auth is via bearer token or basic auth on a shared `CollibraClient` that holds the long-lived URL + credentials (per-call scope is passed to `.ingest()` / `.extract()`).
* Add Collibra subtype nodes to the RDBMS data model (`CollibraDatabase`, `CollibraSchema`, `CollibraTable`, `CollibraColumn`, `CollibraGlossary`, `CollibraCategory`, `CollibraBusinessTerm`) — subclasses of the core node types written with a secondary graph label (e.g. `:Table:CollibraTable`), carrying the source `collibra_id` (and `status` / `collibra_asset_type` where applicable). This keeps Collibra-specific fields off the shared core models, mirroring the OSI subtype approach. Also adds `CollibraTaggedWith`, whose `TAGGED_WITH` edge matches the tagged Table/Column by `collibra_id` so it resolves across the schema and glossary sub-connectors.
* Add `create_range_index` to `neocarta.ingest.indexes` (generic property range index); the Collibra loader creates a `collibra_id` range index per `Collibra*` label so `TAGGED_WITH` endpoint lookups seek rather than scan.
* Add `UnmappedCollibraAssetTypeWarning` (in `neocarta.warnings`), emitted once per run for first-class Collibra asset types outside a sub-connector's scope (Report, Data Quality Rule, Policy, …), which are skipped rather than coerced into an ill-fitting node. Technical lineage and first-class modelling of those asset types are deferred to follow-up PRs.
