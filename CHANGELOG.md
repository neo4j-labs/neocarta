...
- Embed a composed `name | type | description` text for every node instead of the bare `description`, making embeddings consistent across all connectors and both Databricks embedding modes. The shared enrichment path (`get_nodes_to_embed`), used by external embedding and the BigQuery, CSV, and Dataplex connectors, previously embedded only `n.description` and skipped any node whose description was null or empty. The Databricks inline Spark path embedded a different, richer text (a catalog-qualified name plus type and comment). The two diverged: a Databricks graph embedded different text inline versus externally, and nodes without a comment, including every Database node, embedded nothing in external mode. `get_nodes_to_embed` now composes `name | type | description`, joining with `` | ``, dropping null or blank parts, with `type` present only on Column nodes, and selects nodes by `embedding IS NULL` alone, so a node always embeds on at least its name. The Databricks inline `EMBEDDING_TEXT_EXPR` is aligned to the same `name | type | comment` shape, so inline and external embed identical text. Only nodes missing an `embedding` are processed, so existing vectors are untouched; re-embed to apply the composed text to already-embedded nodes. See `neocarta/enrichment/embeddings/README.md`.

### Changed

- Change the Databricks connector's node-identity scheme to a collision-safe hash. Each node's `id` (the Neo4j MERGE key) is now `md5(catalog.schema.table.column)` over the lossless lowercased path, and a new readable `qualified_name` property carries the dotted path itself (on Database/Schema/Table/Column). Previously the `id` was the dotted path with hyphens and spaces folded to underscores, which is not injective: two distinct Unity Catalog objects differing only by `-` vs `_` (e.g. `graph-enriched-schema` vs `graph_enriched_schema`) collapsed to one id and were silently merged into a single node. The new builders live in `neocarta/connectors/databricks/ingest/contract_expr.py` (byte-identical Python `node_id`/`qualified_name` and Spark `node_id_expr`/`qualified_name_expr`) and no longer depend on the shared `neocarta.connectors.utils.generate_id` helpers, which the other connectors still use. This is a breaking change to the Databricks graph's node ids, so an existing Databricks-ingested graph must be re-ingested; in practice this is a no-op since every run is a clean rebuild. See `neocarta/connectors/databricks/README.md`.
- Pin the Databricks connector's runtime dependencies to the tested versions with upper bounds, so the published `neocarta[databricks-spark]` wheel resolves the same way locally and on a cluster: `pyspark>=4.1.2,<4.2` and `pydantic-settings>=2.12.0,<2.13` (the `databricks-spark` extra), and the shared base deps `neo4j>=6.2.0,<6.3`, `pydantic>=2.12.5,<2.13`, and `databricks-sdk>=0.114.0,<0.115` (the last raises the floor from `0.40`). Resolution is unchanged from the prior lock; the bounds prevent silent drift to an untested major/minor.
- Move the Databricks connector's Neo4j constraint/index DDL out of `ingest/load/neo4j_io.py` into a new `ingest/load/indexes.py` module (`bootstrap_constraints`, `create_vector_indexes`), leaving `neo4j_io.py` to own only the data writes. Only inline mode creates the per-label `{label}_vector_index` cosine indexes during the Spark job (at `NEOCARTA_DATABRICKS_EMBEDDING_DIMENSION`, one index per label whose embedding flag is on), because only inline writes embeddings in the job. External mode creates no vector indexes during ingest; the `neocarta databricks embed` CLI creates each index at the dimension it actually embeds (see `neocarta/enrichment/embeddings/`), so the index dimension always matches the stored vectors. Previously the Spark job created vector indexes in *both* modes, which in external mode locked the index to the job's configured dimension before any embedding existed — if the later CLI embed produced a different dimension, its `IF NOT EXISTS` index create was a no-op and vector search returned 0 results. Value nodes are never indexed. See `neocarta/connectors/databricks/README.md`.

### Added

- Add the Schema/Table/Column full-text indexes to the Databricks connector, so keyword search works on a Databricks-ingested graph. The connector's index bootstrap now creates `schema_full_text_index`, `table_full_text_index`, and `column_full_text_index` via the shared `neocarta.ingest.indexes.create_full_text_index` helper, matching what the other connectors (BigQuery, CSV, Dataplex) create through the RDBMS loader and what the MCP server's full-text Cypher queries by. Previously the connector created only id constraints, range indexes, and vector indexes, so the hybrid retrieval's keyword half returned nothing on a Databricks graph: table and column names were neither full-text indexed nor (before the embedding fix above) embedded. Each index covers `name`, `qualified_name`, and `description`; `qualified_name` (the dotted `catalog.schema.table` path) is included so keyword search can disambiguate by catalog/schema, since Lucene tokenizes the dotted path into separate words while the bare name still matches exactly. This is lexical-only — embeddings stay on the bare `name | type | comment` text. The indexes are created with `IF NOT EXISTS` (idempotent) and independently of the embedding mode. Database is intentionally not full-text indexed, matching the other connectors. See `neocarta/connectors/databricks/README.md`.
- Add streamable HTTP transport to the `neocarta-mcp` server, alongside the default stdio. New flags on the entry point: `--http` (serve over streamable HTTP instead of stdio), `--host` (default `127.0.0.1`), `--port` (default `8000`), and `--path` (default `/mcp`). Without `--http` the server runs over stdio exactly as before, so an MCP client can still launch it as a subprocess; with `--http` it runs independently and clients connect by URL (e.g. `http://127.0.0.1:8000/mcp`).
- Add `scripts/test-neocarta-mcp.py`: a standalone `uv` smoke client (PEP 723 inline deps) that connects to a running neocarta MCP server over streamable HTTP (`--url`), lists every registered tool with its schema, and probes each one with a sensible argument (catalog tools with no/discovered-schema args, search tools with the `--query` string), printing a result preview and a pass/fail/skip summary. It exits non-zero if any probed tool errors, so it doubles as a health check. See `scripts/README.md` for the two-terminal run flow.
...
- Add `scripts/setup_openai_external_model_endpoint.py`: a standalone `uv` helper (PEP 723 inline deps) that registers a Databricks Mosaic AI "External Model" serving endpoint proxying OpenAI `text-embedding-3-small` (1536-dim), reading the OpenAI key from a Databricks secret (`neocarta-openai/OPENAI_API_KEY` by default), then probes it through the same `ai_query` path the connector uses to assert the returned vector dimension. The created endpoint is callable with `ai_query` like a native model, so pointing `NEOCARTA_DATABRICKS_EMBEDDING_ENDPOINT` at it makes inline embeddings run on OpenAI and match an OpenAI-based neocarta graph. This automates the "Registering the OpenAI endpoint (manual step)" section of `neocarta/connectors/databricks/README.md`. Run with `uv run scripts/setup_openai_external_model_endpoint.py --profile <profile>`; `--skip-verify` creates the endpoint without the probe.
