...
- `neocarta databricks glossary`: new CLI command wrapping `DatabricksGlossaryConnector`, exposing the Databricks governed-tags glossary source on the CLI alongside `bigquery`, `csv`, `dataplex`, `jdbc`, `osi`, and `query-log`. The workspace URL resolves from `--host` or `DATABRICKS_HOST`; the access token is read only from `DATABRICKS_TOKEN` (secret), never a flag, matching the JDBC connector's secret handling. Supports `--include-system-tags`, `--glossary-id` / `--glossary-name` overrides, opt-in `--embeddings` (over `BusinessTerm` nodes), `--dry-run`, and the standard JSON/exit-code contract; the new `DATABRICKS_HOST` / `DATABRICKS_TOKEN` env vars are surfaced through `neocarta agent-context`.

- `DatabricksGlossaryConnector` (`neocarta.connectors.databricks`): a source connector that ingests business-glossary metadata from **managed Databricks** Unity Catalog **governed tags** (account-level controlled vocabularies). It reads governed-tag *definitions* via the Databricks SDK (`WorkspaceClient.tag_policies.list_tag_policies()`) — no SQL warehouse, no cluster, no `information_schema` — and maps each governed tag onto the existing glossary layer: one synthesized account/metastore-level `Glossary` (id derived from the workspace metastore id, falling back to the host with a `DatabricksGlossaryWarning`), each governed tag *key* → a `Category` (carrying the tag's description and tag-policy id), and each allowed *value* → a `BusinessTerm`. Allowed values carry no description in Databricks, so `BusinessTerm` nodes are written name-only — no description is fabricated (loaded with `properties_list=["name"]`). Platform-managed `system.*` governed tags are excluded by default (`include_system_tags=True` to include them); value-less governed tags become a `Category` with no `BusinessTerm` children. This is distinct from the open-source-UC `UnityCatalogSchemaConnector` (governed tags are a managed-Databricks feature), so it ships in a new `neocarta.connectors.databricks` package behind a new optional **`databricks`** extra (`pip install neocarta[databricks]`, `databricks-sdk>=0.118.0`) — a deliberate departure from the core-dependency convention used for BigQuery/Dataplex, chosen so a future pyspark-based Databricks schema connector can ship in a separate extra and glossary users never pull Spark. **v1 reads definitions only:** tag *assignments* (`information_schema.{catalog,schema,table,column}_tags` → `(:Column|:Table|:Schema)-[:TAGGED_WITH]->(:BusinessTerm)` edges, which require a SQL warehouse and power the MCP business-term-bridged search) are a planned follow-up behind an `include_assignments` flag. Ships unit tests (mocked `tag_policies`, no live workspace) and `examples/databricks_glossary.py`, with `DATABRICKS_HOST` / `DATABRICKS_TOKEN` in `.env.example`.
