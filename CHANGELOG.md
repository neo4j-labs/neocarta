...
### Added
- `neocarta mcp <tool>`: every Neocarta MCP server tool is now also a first-class CLI command, mirroring the server by **name, arguments, and documentation** (#164). The new `neocarta mcp` group exposes the three catalog tools (`list-schemas`, `list-tables-by-schema`, `get-full-metadata-schema`) and all nine search tools (`get-context-by-{table,column}-{vector,full-text,hybrid,business-term-hybrid}-search` plus `get-context-by-schema-and-table-vector-search`); each command name is the kebab-cased tool name, each `--help` is the tool's docstring, and the `--text-content` / `--max-tables` / `--search-top-k` flags (with the same per-tool defaults) and `--schema-name` mirror the tool signatures, all surfaced through `neocarta agent-context`. The commands reuse the same Cypher builders (`neocarta._mcp.cypher`), result models (`neocarta._mcp.models`), Lucene sanitiser, and `LiteLLMEmbeddingsConnector` the server uses, run synchronously against the CLI's Neo4j driver, and emit the standard `{ "mcp_<tool>": { ..., "count", "results" } }` JSON envelope. Unlike the server's async, index-gated single-tool-per-label registration, the CLI exposes all tools statically; a search command run against a graph missing the required vector/full-text index fails cleanly with exit code `3` (`not_found`), and a failed query embedding maps to `8` (`upstream_error`). The catalog commands need only `[cli]` and `NEO4J_*`; the embedding-backed search commands additionally need an embedding-provider key (e.g. `OPENAI_API_KEY`) and read `EMBEDDING_MODEL` / `EMBEDDING_DIMENSIONS` so query and stored vectors agree. No `fastmcp` / `[mcp]` extra is required to call these commands. Ships unit tests in `tests/unit/_cli/test_mcp.py`. Closes #164.
