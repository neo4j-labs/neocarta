...
- `UnityCatalogSchemaConnector` (`neocarta.connectors.unity_catalog`): a source connector that ingests structural metadata (`Database`, `Schema`, `Table`, `Column` nodes plus `HAS_SCHEMA` / `HAS_TABLE` / `HAS_COLUMN` edges) from the **open** Unity Catalog REST API (`/api/2.1/unity-catalog`, the Apache-2.0 project at github.com/unitycatalog/unitycatalog — not the Databricks SDK), so it works against any conformant Unity Catalog server. Catalogs map to `Database`, schemas to `Schema`, tables and views to `Table` (the UC `table_type` is recorded during extraction but views are not modeled as a distinct node label), and each table's inline `columns` to `Column`. Transport is a thin owned `httpx.Client` (bearer token optional — a local OSS server needs none), so `httpx` is now a direct dependency. The constructor takes `base_url` and an optional `token`; the catalog (and an optional `schemas` filter) is passed per call to `ingest(catalog=..., schemas=[...])`. Extraction is scoped with the shared `include_nodes` / `include_relationships` enums (tables are fetched transiently to attach columns even when `TABLE` is excluded), list endpoints are paginated defensively via `next_page_token`, and HTTP failures map to typed errors (`ConnectorError` for an unreachable host, `ExtractionError` for a non-2xx response or a malformed payload missing a required `name`, `OperationTimeoutError` for timeouts; the token and response bodies are never logged). The connector owns its `httpx.Client` and is usable as a context manager (or via `close()`) to release the connection pool. The open UC API exposes no primary/foreign-key constraints, so `is_primary_key` / `is_foreign_key` are left null (unpopulated) and no `REFERENCES` edges are produced; no value sampling or glossary/tags are performed. Ships conformance, extract, and transform unit tests driven by recorded JSON fixtures (httpx mocked via `unittest.mock`) plus a fixture-backed integration test (`tests/integration/connectors/unity_catalog/`), and `examples/unity_catalog.py` with `UC_*` entries in `.env.example`. CLI wiring and a glossary sub-connector (sourced from `INFORMATION_SCHEMA`, which the REST API does not expose) are separate follow-ups. See `neocarta/connectors/unity_catalog/README.md`.
