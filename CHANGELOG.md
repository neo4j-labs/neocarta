...
- `dimensions` is now a first-class constructor argument on `LiteLLMEmbeddingsConnector` (mirroring `OpenAIEmbeddingsConnector`), replacing the need to pass it through `litellm_kwargs`. When set it is forwarded to LiteLLM together with `drop_params=True`; advanced OpenAI-compatible endpoints that LiteLLM cannot introspect can still force the drop via `litellm_kwargs={"additional_drop_params": ["dimensions"]}`.
