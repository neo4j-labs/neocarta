.PHONY: help agent create-graph create-graph-no-embeddings clean test-databricks build databricks-wheel-test
...
refresh-mermaid-data-model-images:
	...
	mmdc -i assets/mermaid/data_model/osi-data-model-1.mmd -o assets/images/data_model/osi-data-model-1.png
...
test-unit:
	uv run pytest tests/unit -v --ignore=tests/unit/_mcp --ignore=tests/unit/_cli
