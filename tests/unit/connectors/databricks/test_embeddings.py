"""Spark-logic tests for the inline embedding transform, gate, and ledger split.

Exercises `_validate_embedding` directly with synthesized `_emb_raw` structs so
the wrong-shape and endpoint-error paths are covered without calling ai_query
(a Databricks-only SQL function), plus the per-batch failure gate and the
ledger join logic. These run under the ``databricks`` dependency group against a
local SparkSession; the top-level ``pyspark`` import is guarded with
``importorskip`` so collection in the default ``test-unit`` group skips this
module rather than erroring.
"""
...
from neocarta.connectors.databricks.ingest.transform.embeddings import (
    _validate_embedding,
...
def test_wrong_shape_row_counted_as_failure(local_spark) -> None:
    ...
    from neocarta.connectors.databricks.ingest.transform.embeddings import compute_failure_stats
...
def _gate_inputs(local_spark, n_fail: int, n_ok: int, cap: int):
    ...
    frame, `embedding_failure_max` off settings, and the attempts/successes
    dicts off `summary.embeddings`, so lightweight stubs suffice.
    """
    from types import SimpleNamespace

    from neocarta.connectors.databricks.ingest.summary import EmbeddingCounts

    rows = [([0.1, 0.2],) for _ in range(n_ok)] + [(None,) for _ in range(n_fail)]
    df = local_spark.createDataFrame(rows, "embedding array<double>")
    settings = SimpleNamespace(embedding_failure_max=cap)
...
def test_per_batch_count_gate_trips_when_failures_exceed_cap(local_spark) -> None:
    """The per-batch gate raises before the batch is written when this batch's
    failure count exceeds NEOCARTA_DATABRICKS_EMBEDDING_FAILURE_MAX, and
    accumulates this batch's attempts/successes onto the summary."""
    from neocarta.connectors.databricks.ingest.transform.embed_stage import _accumulate_and_gate
    from neocarta.enums import NodeLabel

    df, settings, summary = _gate_inputs(local_spark, n_fail=3, n_ok=7, cap=2)
    with pytest.raises(RuntimeError, match="NEOCARTA_DATABRICKS_EMBEDDING_FAILURE_MAX"):
...
def test_per_batch_count_gate_passes_under_cap_and_when_disabled(local_spark) -> None:
...
def test_ledger_all_rows_hit_misses_empty(local_spark) -> None:
    ...
    from neocarta.connectors.databricks.ingest.transform.ledger import split_by_ledger
