"""Conformance tests for DatabricksMetricsConnector.
...
METRICS_PACKAGE = "neocarta.connectors.databricks.metrics"


def _make_connector() -> DatabricksMetricsConnector:
    """Construct a DatabricksMetricsConnector with mocked external dependencies."""
    return DatabricksMetricsConnector(
...
def test_conforms_to_source_connector_protocol():
    """DatabricksMetricsConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(DatabricksMetricsConnector, name)), f"missing public method: {name}"
...
def test_readme_present():
    ...
    module = importlib.import_module(METRICS_PACKAGE)
...
def test_init_still_exports_existing_connectors():
    """Adding the metrics connector must not drop the existing schema/tags exports."""
    module = importlib.import_module(PACKAGE)
    assert "DatabricksMetricsConnector" in module.__all__
...
def test_metrics_subpackage_exports_are_minimal():
    """The metrics sub-package's own __init__.py exports only its connector class."""
    module = importlib.import_module(METRICS_PACKAGE)
    exported = getattr(module, "__all__", None)
    assert exported is not None, "metrics/__init__.py must define __all__"
    assert "DatabricksMetricsConnector" in exported
    for name in exported:
        assert not name.endswith(("Extractor", "Transformer", "Loader")), (
            f"{name} should not be re-exported from {METRICS_PACKAGE}.__init__.py"
        )
...
def test_missing_connection_raises_config_error():
    ...
    with pytest.raises(ConfigError):
        DatabricksMetricsConnector(
            connection=None, catalog="test_catalog", neo4j_driver=MagicMock()
        )
...
def test_missing_catalog_raises_config_error():
    ...
    with pytest.raises(ConfigError):
        DatabricksMetricsConnector(connection=MagicMock(), catalog="", neo4j_driver=MagicMock())
...
def test_missing_driver_raises_config_error():
    ...
    with pytest.raises(ConfigError):
        DatabricksMetricsConnector(
            connection=MagicMock(), catalog="test_catalog", neo4j_driver=None
        )
...
def test_extract_rejects_backtick_schema():
    """A malformed schema identifier fails fast."""
    connector = _make_connector()
    with pytest.raises(ConfigError):
