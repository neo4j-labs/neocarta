"""Conformance tests for DatabricksTagsConnector.
...
def _make_connector() -> DatabricksTagsConnector:
    """Construct a DatabricksTagsConnector with mocked external dependencies."""
    return DatabricksTagsConnector(
...
def test_conforms_to_source_connector_protocol():
    """DatabricksTagsConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(DatabricksTagsConnector, name)), f"missing public method: {name}"
