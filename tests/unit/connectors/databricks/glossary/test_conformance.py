"""Conformance tests for DatabricksGlossaryConnector.
...
def _make_connector() -> DatabricksGlossaryConnector:
    """Construct a DatabricksGlossaryConnector with mocked external dependencies."""
    return DatabricksGlossaryConnector(
...
def test_conforms_to_source_connector_protocol():
    """DatabricksGlossaryConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(DatabricksGlossaryConnector, name)), (
