"""Conformance tests for DataplexSchemaConnector.
...
def _make_connector() -> DataplexSchemaConnector:
    """Construct a DataplexSchemaConnector with mocked external dependencies."""
...
def test_conforms_to_source_connector_protocol():
    """DataplexSchemaConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(DataplexSchemaConnector, name)), f"missing public method: {name}"
