"""Conformance tests for BigQueryLogsConnector.
...
def _make_connector() -> BigQueryLogsConnector:
    """Construct a BigQueryLogsConnector with mocked external dependencies."""
    client = MagicMock()
    client.project = "test-project"
    return BigQueryLogsConnector(client=client, project_id="test-project", neo4j_driver=MagicMock())


def test_conforms_to_source_connector_protocol():
    """BigQueryLogsConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(BigQueryLogsConnector, name)), f"missing public method: {name}"
...
def test_run_emits_deprecation_warning():
    ...
    with pytest.warns(DeprecationWarning, match="run"):
        connector.run(dataset_id="dataset")
