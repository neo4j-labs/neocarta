"""Conformance tests for BigQuerySchemaConnector.
...
PACKAGE = "neocarta.connectors.bigquery"


def _make_connector() -> BigQuerySchemaConnector:
    """Construct a BigQuerySchemaConnector with mocked external dependencies."""
    client = MagicMock()
    client.project = "test-project"
    return BigQuerySchemaConnector(
        client=client, project_id="test-project", neo4j_driver=MagicMock()
    )


def test_conforms_to_source_connector_protocol():
    """BigQuerySchemaConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(BigQuerySchemaConnector, name)), f"missing public method: {name}"
...
def test_dataset_id_in_init_emits_deprecation_warning():
    """Passing dataset_id to __init__ is deprecated; warning must fire."""
    client = MagicMock()
    client.project = "test-project"
    with pytest.warns(DeprecationWarning, match="dataset_id"):
