"""Conformance tests for CSVConnector.
...
PACKAGE = "neocarta.connectors.csv"


def _make_connector(tmp_path: pathlib.Path | None = None) -> CSVConnector:
    """Construct a CSVConnector with mocked external dependencies."""
    csv_directory = str(tmp_path) if tmp_path is not None else "."
    return CSVConnector(csv_directory=csv_directory, neo4j_driver=MagicMock())


def test_conforms_to_source_connector_protocol():
    """CSVConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(CSVConnector, name)), f"missing public method: {name}"


def test_run_emits_deprecation_warning():
    """run() must emit DeprecationWarning and delegate to ingest()."""
    connector = _make_connector()
    connector.ingest = MagicMock()  # silence real ingest
