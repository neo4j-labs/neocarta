...
PACKAGE = "neocarta.connectors.query_log"


def _make_connector() -> QueryLogConnector:
    """Construct a QueryLogConnector with a mocked Neo4j driver."""
    return QueryLogConnector(neo4j_driver=MagicMock())


def test_conforms_to_source_connector_protocol():
    """QueryLogConnector is a source connector."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(QueryLogConnector, name)), f"missing public method: {name}"
