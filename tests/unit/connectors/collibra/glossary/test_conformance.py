"""Conformance tests for CollibraGlossaryConnector.
...
# README lives at the connector package root (shared by both sub-connectors).
PACKAGE = "neocarta.connectors.collibra"


def _make_connector() -> CollibraGlossaryConnector:
    """Construct a CollibraGlossaryConnector with mocked external dependencies."""
    return CollibraGlossaryConnector(client=MagicMock(), neo4j_driver=MagicMock())


def test_conforms_to_source_connector_protocol():
    """CollibraGlossaryConnector conforms to the SourceConnectorProtocol."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(CollibraGlossaryConnector, name)), f"missing public method: {name}"
