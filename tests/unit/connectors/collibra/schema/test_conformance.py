"""Conformance tests for CollibraSchemaConnector.
...
def _make_connector() -> CollibraSchemaConnector:
    """Construct a CollibraSchemaConnector with mocked external dependencies."""
    return CollibraSchemaConnector(client=MagicMock(), neo4j_driver=MagicMock())


def test_conforms_to_source_connector_protocol():
    """CollibraSchemaConnector conforms to the SourceConnectorProtocol."""
...
def test_has_public_stage_methods():
    ...
    for name in ("extract", "transform", "load", "ingest", "run"):
        assert callable(getattr(CollibraSchemaConnector, name)), f"missing public method: {name}"
