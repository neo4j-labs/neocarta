"""Unit tests for ``neocarta csv load``.
...
def test_load_help_documents_key_flags():
    runner = CliRunner()
    result = runner.invoke(cli, ["csv", "load", "--help"])
...
def test_load_dry_run_emits_json_and_skips_clients():
    runner = CliRunner()
    ...
    body = payload["csv_load"]
    assert body["dry_run"] is True
    assert body["csv_directory"] == "datasets/csv"
    assert body["embeddings"] is False


def test_load_missing_csv_directory_fails_with_usage_error(monkeypatch):
    ...
    monkeypatch.delenv("CSV_DIRECTORY", raising=False)
    runner = CliRunner()
    result = runner.invoke(cli, ["--json", "csv", "load", "--dry-run"])
    # CLIError("usage_error") → exit code 2 per the AGENTS-CLI map.
    assert result.exit_code == 2, result.stdout + result.stderr
    payload = json.loads(result.stdout)
    assert payload["error"]["code"] == "usage_error"
    assert "csv-directory" in payload["error"]["message"].lower()


@pytest.fixture
def _cli_env(monkeypatch):
    """Populate the env vars required for the csv command to start."""
...
def test_load_success_emits_json():
    ...
    assert payload["csv_load"]["status"] == "succeeded"
    assert payload["csv_load"]["embeddings"] is False


@pytest.mark.parametrize(
    ("error", "expected_exit_code"),
    [
        (ConfigError("csv_directory does not exist"), EXIT_CODES["validation_error"]["code"]),
...
@pytest.mark.usefixtures("_cli_env")
def test_csv_load_routes_library_errors_to_exit_codes(
    error: NeocartaError, expected_exit_code: int
):
    """Every NeocartaError raised from the CSV connector becomes its CLI code."""
    runner = CliRunner()
    with (
        patch("neocarta._cli.commands.csv._neo4j_driver") as mock_driver_ctx,
        patch("neocarta.connectors.csv.CSVConnector", side_effect=error),
    ):
        mock_driver_ctx.return_value.__enter__.return_value = MagicMock()
        mock_driver_ctx.return_value.__exit__.return_value = False

        result = runner.invoke(cli, ["csv", "load"])
