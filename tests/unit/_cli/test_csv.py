"""Unit tests for ``neocarta csv ingest``.
...
def test_ingest_missing_csv_directory_fails_with_usage_error(monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["--json", "csv", "ingest", "--dry-run"])
...
def test_ingest_success_emits_json():
    ...
    with (
        ...
        result = runner.invoke(cli, ["--json", "csv", "ingest"])
...
@pytest.mark.usefixtures("_cli_env")
def test_csv_ingest_routes_library_errors_to_exit_codes(
    ...
    with (
        ...
        result = runner.invoke(cli, ["csv", "ingest"])
