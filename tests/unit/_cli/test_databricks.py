"""Unit tests for ``neocarta databricks glossary``.
...
def test_group_and_help_document_glossary_verb():
    runner = CliRunner()
    group = runner.invoke(cli, ["databricks", "--help"])
    assert group.exit_code == 0
    assert "glossary" in group.output

    result = runner.invoke(cli, ["databricks", "glossary", "--help"])
...
def test_agent_context_registers_databricks_glossary():
    runner = CliRunner()
    payload = json.loads(runner.invoke(cli, ["agent-context"]).output)
    assert "databricks" in payload["commands"]
    assert "glossary" in payload["commands"]["databricks"]["subcommands"]
...
def test_dry_run_emits_json_without_token_or_clients(monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["--json", "databricks", "glossary", "--dry-run"])
    assert result.exit_code == 0, result.output
    body = json.loads(result.output)["databricks_glossary"]
...
def test_success_forwards_token_to_client_and_hides_it(monkeypatch):
    ...
    with (
        ...
        patch(
            "neocarta.connectors.databricks.DatabricksGlossaryConnector",
        ...
        result = runner.invoke(cli, ["--json", "databricks", "glossary"])

    assert result.exit_code == 0, result.output
    out = result.output
    payload = json.loads(out[out.index("{") :])
    assert payload["databricks_glossary"]["status"] == "succeeded"
...
def test_routes_library_errors_to_exit_codes(monkeypatch, error, expected_exit_code):
    ...
    with (
        ...
        patch("neocarta.connectors.databricks.DatabricksGlossaryConnector", side_effect=error),
    ):
        mock_driver_ctx.return_value.__enter__.return_value = MagicMock()
        mock_driver_ctx.return_value.__exit__.return_value = False
        result = runner.invoke(cli, ["databricks", "glossary"])
